...
class Simulator {
	...
	public static void main(String[] args)
	{
		...
		try {
			...
			System.out.println(group_set.size());
		...
		for (int i = 0 ; i != score.length ; ++i) {
            double s = m.getOrDefault(groups[i], 0.0);
            int j = m2.getOrDefault(groups[i], 0);
            s += score[i];
            j += 1;
        ...
        for (String s : m.keySet()) {
            System.err.println(s + " mean score: " + m.get(s) / m2.get(s));
        }
		System.exit(0);
	}
	                         ...
	                         boolean verbose) throws Exception
	{
        ...
        int maxscore[] = new int[W.length];
        int maxlscore[] = new int[W.length];
        for (int i = 0; i < W.length; ++i) {
            int s = 0;
            int s2 = 0;
            for (int j = 0; j < W[0].length; ++j) {
                s += Math.min(W[i][j], W[j][i]);
                s2 += W[j][i];
            }
            maxscore[i] = s2;
            maxlscore[i] = s;
        ...
        for (int i = 0; i < W.length; ++i) {
            System.err.println(i + "|\t\t" + score[i] + "\t" + maxlscore[i] + "\t" + maxscore[i] + "\t");
        }
